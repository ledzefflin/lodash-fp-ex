 ...
 */
const then = fp.curry((fn, thenable) =>
  promisify(thenable).then(flatPromise(fn)),
);
 ...
 */
const otherwise = fp.curry((fn, thenable) =>
  promisify(thenable).catch(flatPromise(fn)),
);
 ...
 * @return {void} 반환값 없음
 */
const _finally = fp.curry((fn, thenable) =>
  promisify(thenable).finally(flatPromise(fn)),
);
...
const transformObjectKey = fp.curry((transformFn, dest) => {
  ...
  const result =
    fp.isObject(dest) || fp.isArray(dest) ? convertRecursively(dest) : dest;
 ...
 * @returns {void} 반환값 없음
 */
const forEachWithKey = fp.curry((f, a) =>
  fp.forEach.convert({ cap: false })(f, a),
);
 ...
 * @returns {any} 누적기
 */
const reduceWithKey = fp.curry((f, acc, a) =>
  fp.reduce.convert({ cap: false })(f, acc, a),
);
 ...
 */
const isVal = (a) =>
  fp.isNil(a) || fp.isBoolean(a) || fp.isNumber(a) || fp.isString(a);
