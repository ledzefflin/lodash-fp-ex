...
const key = _.curry((a, v) => {
  const getKey = (a, b) => _.isString(a) ? a : b;
  const composer = _.pipe(_.invert, _.get(getKey(a, v)));
  const result = composer(!_.isString(a) ? a : v);
...
const normalize = _.pipe(_.toString, _.toLower, _.trim);
